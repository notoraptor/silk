 ...
 *
 */
/* Modifications par Steven Bocco (08 Février 2016).
Suppression des retours de lignes superflus.
Les modifications particulières sont précédées par des commentaires unilignes débutant avec "//--".
*/

/**-- Test.
$infos = SimpleCaptcha::generate();
echo "<p>$infos[captcha]</p>";
echo '<p><img src="data:image/'.$infos['type'].';base64,'.$infos['image'].'"/></p>';
print_r($infos);
file_put_contents("essai.html", '<p><img src="data:image/'.$infos['type'].';base64,'.$infos['image'].'"/></p>');
*/
...
class SimpleCaptcha {
	...
	//-- La génération de l'image et du code CAPTCHA ne doivent plus dépendre d'une session.
	...
	//-- L'image est désormais créée avec un texte CAPTCHA spécifié manuellement.
    public function CreateImage($text) {
        ...
        /** Text insertion */
		//-- Initialisation du texte affiché dans l'image CAPTCHA.
        //$text = $captchaText ? $captchaText : $this->GetCaptchaText();
	...
	//--
	public function getSomeCaptchaText() {
